...
public class EgovMmsServiceImpl extends AbstractServiceImpl implements EgovMmsService {
	...
	public Map<String, Object> sendMms(MmsTransInfo mmsTransInfo) throws Exception {
		...
		try {
			...
			if(response.isSuccess()) { 
				...
				log.info("ok 메시지 ID : [" + messageId + "]");
				log.info("ok 상태 정보 : [" + status + "]");
				...
				log.info("no 실패 코드 : [" + faultCode + "]");
				log.info("no 실패 설명 정보 : [" + faultString + "]");
				log.info("no 실패 액터 정보 : [" + faultActor + "]");
			}
			...
			log.error("IllegalArgumentException has occurred", ie);
		} catch (Exception e) {
	...
	public Map<String, Object> receiveMmsTransmissionResult(HttpServletRequest request) throws Exception {
		...
		try{
			...
			for(int i = 0; i < msgList.length; i++) {
				...
				log.info("RECEIVE_NO=" + receiveNo);
				log.info("RECIPIENT=" + recipient);
				log.info("FORWARD_DATE=" + forwardDate);
				log.info("DELIVERY_DATE=" + deliveryDate);
				log.info("KTF_STATUS=" + ktfStatus);
				log.info("LGT_STATUS=" + lgtStatus);
				log.info("SKT_STATUS=" + sktStatus);
			...
			log.error("Exception has occurred in " + this.getClass().getName(), e);
		}
	...
	public Map<String, Object> getMmsTransmissionResult(String messageId) throws Exception {
		...
		try {
			...
			for (int i = 0; i < msgList.length; i++) {
				...
				log.info("TELCO=" + telco);
				log.info("RECEIVE_NO=" + receiveNo);
				log.info("RECIPIENT=" + receiveName);
				log.info("DELIVERY_DATE=" + deliveryDate);
				log.info("KTF_STATUS=" + ktfStatus);
				log.info("LGT_STATUS=" + lgtStatus);
				log.info("SKT_STATUS=" + sktStatus);
